...
const SWATCHES = [
  'var(--accent)', '#cba6f7', '#89b4fa', '#a6e3a1',
  '#f9e2af',       '#fab387', '#f38ba8', 'var(--muted)',
]
...
function FastFetchTerminal() {
  ...
  useEffect(() => {
    ...
    if (phase === 'typing') {
      setCmdDisplayed(''); setVisibleRows(0); setShowSwatches(false)
      setRmDisplayed(''); setGlitch(false); setDestroyed(false)
      let i = 0
      const typeNext = () => {
        i++; setCmdDisplayed(CMD.slice(0, i))
    ...
    if (phase === 'rm_typing') {
      let i = 0
      const typeNext = () => {
        i++; setRmDisplayed(RM_CMD.slice(0, i))
  ...
  if (destroyed) {
    return (
      <div style={{
        ...
        <span style={{ color: '#f38ba8', fontSize: '0.72rem', opacity: 0.4 }}>terminal closed</span>
      </div>
    )
  }
  ...
  return (
    <div style={{
      ...
      borderRadius: '12px',
      fontFamily: "'Fira Code','Cascadia Code','JetBrains Mono','Courier New',monospace",
      fontSize: '0.78rem', lineHeight: 1.9, minHeight: '260px', overflow: 'hidden',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
        ...
        <span style={{ color: 'var(--accent)', marginLeft: '4px' }}>{cmdDisplayed}</span>
        {fetchCursor && (
      ...
      {showFetchOutput && visibleRows > 0 && (
        <div style={{ marginTop: '6px' }}>
          <div style={{
            color: 'var(--accent)', whiteSpace: 'pre', fontSize: '0.58rem',
            lineHeight: 1.35, opacity: 0.9, marginBottom: '10px',
            fontFamily: "'Fira Code','Cascadia Code','JetBrains Mono','Courier New',monospace",
          }}>
            {ASCII_LOGO}
          </div>
          ...
          {showSwatches && (
            <div style={{ display: 'flex', gap: '5px', marginTop: '12px', paddingTop: '8px', borderTop: '1px solid var(--border)' }}>
              {SWATCHES.map((c, i) => (
...
export default function About() {
  return (
    <section id="about" style={{ padding: '100px max(24px, calc((100vw - 900px) / 2))', borderTop: '1px solid var(--border)' }}>
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
        <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent)', fontSize: '0.8rem', marginBottom: '8px', letterSpacing: '0.05em', opacity: 0.6 }}>$ whoami</div>
        <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 400, fontStyle: 'italic', fontFamily: 'var(--font-display)', color: 'var(--text)', marginBottom: '32px', letterSpacing: '-0.01em' }}>about me</h2>

        <div style={{ display: 'flex', gap: '40px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div style={{ width: '120px', height: '120px', borderRadius: '50%', background: 'var(--surface)', border: '2px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem', flexShrink: 0 }}>🧑‍💻</div>
          <div style={{ flex: 1, minWidth: '260px' }}>
            <p style={{ color: 'var(--text)', fontSize: '0.95rem', fontFamily: 'var(--font-body)', lineHeight: 1.8, marginBottom: '24px' }}>
              I&apos;m Adarsh, a student at NIT Rourkela. I&apos;m into programming, math, and problem-solving,
              and I run Arch btw. This site is where I write notes and things I find interesting —
              not limited to tech. Outside screens I enjoy music, anime, and reading manhwa and web novels.
            </p>
